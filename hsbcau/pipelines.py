...
class HsbcauPipeline:

    # Database setup
    conn = sqlite3.connect('hsbcau.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `hsbcau`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM hsbcau WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO hsbcau (date, title, link, content)"
